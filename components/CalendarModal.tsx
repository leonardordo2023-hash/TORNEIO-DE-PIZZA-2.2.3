...
export const CalendarModal: React.FC<CalendarModalProps> = ({ isOpen, onClose, language, isAdmin, pizzas, onUpdateDateGlobal }) => {
  ...
  const handleUpdate = (monthCode: string, field: 'day' | 'year', value: string) => {
    ...
    // Se for admin, atualiza a pizza correspondente ou emite broadcast global
  ...
  return (
    ...
    >
      <div 
        className="bg-[#fffbf0] dark:bg-slate-900 w-[90%] max-w-xs sm:max-w-md rounded-3xl shadow-2xl overflow-hidden border border-blue-200 dark:border-slate-700 flex flex-col relative animate-in zoom-in-95 duration-300 max-h-[80vh]"
      ...
      >
        ...
        <div className="bg-blue-100 dark:bg-blue-900/30 p-4 text-center border-b border-blue-200 dark:border-blue-800 relative overflow-hidden shrink-0">
          <button 
            onClick={onClose} 
            className="absolute top-3 right-3 p-1.5 bg-white/50 dark:bg-black/30 rounded-full hover:bg-white dark:hover:bg-black/50 transition-colors text-blue-900 dark:text-blue-100"
          >
            <X size={18} />
          </button>
          
          <div className="flex justify-center mb-2">
            <div className="bg-white dark:bg-slate-800 p-2 rounded-full shadow-sm ring-2 ring-blue-50 dark:ring-blue-900/20">
        ...
        <div className="p-4 overflow-y-auto custom-scrollbar space-y-3 text-slate-700 dark:text-slate-300">
            {isAdmin && (
                <div className="bg-indigo-50 dark:bg-indigo-900/20 p-3 rounded-xl border border-indigo-100 dark:border-indigo-800 flex items-center justify-between mb-2">
                    <span className="text-[10px] font-black uppercase text-indigo-600 dark:text-indigo-400">Modo Admin Ativo</span>
                    <Bell size={14} className="text-indigo-500 animate-wiggle" />
                </div>
            )}
                ...
                <div key={m.code} className={`relative overflow-hidden rounded-2xl border bg-gradient-to-br ${m.color} p-4 shadow-sm transition-all group`}>
                    ...
                    <div className="grid grid-cols-2 gap-2 relative z-10">
                        <div className="bg-white/60 dark:bg-black/20 backdrop-blur-sm p-1 rounded-lg flex items-center shadow-sm">
                            ...
                            <select 
                                disabled={!isAdmin}
                                value={dates[m.code]?.day || ''} 
                                onChange={(e) => handleUpdate(m.code, 'day', e.target.value)} 
                                className="flex-1 bg-transparent font-bold text-sm text-center p-1 focus:outline-none cursor-pointer dark:text-white disabled:opacity-70 disabled:cursor-not-allowed"
                            >
                        ...
                        <div className="bg-white/60 dark:bg-black/20 backdrop-blur-sm p-1 rounded-lg flex items-center shadow-sm">
                            ...
                            <select 
                                disabled={!isAdmin}
                                value={dates[m.code]?.year || ''} 
                                onChange={(e) => handleUpdate(m.code, 'year', e.target.value)} 
                                className="flex-1 bg-transparent font-bold text-sm text-center p-1 focus:outline-none cursor-pointer dark:text-white disabled:opacity-70 disabled:cursor-not-allowed"
                            >
        ...
        <div className="p-3 bg-slate-50 dark:bg-slate-950 border-t border-slate-100 dark:border-slate-800 text-center shrink-0">
            <button 
                onClick={onClose}
                className="w-full bg-slate-900 dark:bg-slate-700 text-white font-black text-xs uppercase py-3 rounded-xl hover:bg-slate-800 transition-colors shadow-md active:scale-[0.98]"
            >
                Confirmar
            </button>
        </div>
