      ...
      try {
          ...
          if (stagedFiles.length > 0) {
              for (let i = 0; i < stagedFiles.length; i++) {
                  const res = await processMediaFile(stagedFiles[i], 1024, (p) => setUploadProgress(Math.round(((i / (stagedFiles.length + (stagedDocument ? 1 : 0))) * 100) + (p / (stagedFiles.length + (stagedDocument ? 1 : 0))))));
                  onAddPhoto(targetId, { id: Math.random().toString(36).substring(2, 15), url: res.url, type: res.type as any, category: 'pizza', date: timestamp + i, caption: i === 0 ? postCaption : "" });
              }
          }
          if (stagedDocument) {
    ...
    <div className="max-w-md mx-auto space-y-4 pb-24 animate-fade-in-up">
      ...
      {isPostModalOpen && (
          <div className="fixed inset-0 z-[500] bg-white/90 dark:bg-slate-900/90 backdrop-blur-md flex items-center justify-center p-2 animate-in fade-in" onClick={resetModal}>
              <div className="bg-white dark:bg-slate-800 w-full max-w-[380px] rounded-[2.2rem] shadow-2xl border-0 overflow-hidden animate-in zoom-in-95 flex flex-col max-h-[95vh] relative" onClick={e => e.stopPropagation()}>
                  <div className="p-4 border-b border-transparent flex justify-between items-center bg-white dark:bg-slate-800 shrink-0">
                      <div className="flex items-center gap-2"><div className="w-8 h-8 bg-indigo-600 rounded-lg flex items-center justify-center text-white shadow-lg rotate-3"><Plus size={16} strokeWidth={3} /></div><h3 className="text-base font-black uppercase tracking-tighter text-slate-800 dark:text-white">Criar Publicação</h3></div>
                      <button onClick={resetModal} className="p-2 bg-slate-100 dark:bg-slate-700 rounded-xl hover:bg-slate-200 transition-all text-slate-500 dark:text-slate-300"><X size={16}/></button>
                  </div>
                  <div className="flex p-1 gap-1 bg-slate-100 dark:bg-slate-900 mx-4 my-2 rounded-xl shrink-0 overflow-x-auto no-scrollbar">
                      <button onClick={() => setPostType('news')} className={`flex-1 min-w-[100px] py-2 rounded-lg text-[9px] font-black uppercase tracking-widest transition-all flex items-center justify-center gap-1 ${postType === 'news' ? 'bg-white dark:bg-slate-800 shadow-sm text-indigo-600' : 'text-slate-400'}`}><Camera size={12} /> Mídia & Arquivos</button>
                      <button onClick={() => setPostType('poll')} className={`flex-1 min-w-[100px] py-2 rounded-lg text-[9px] font-black uppercase tracking-widest transition-all flex items-center justify-center gap-1 ${postType === 'poll' ? 'bg-white dark:bg-slate-800 shadow-sm text-indigo-600' : 'text-slate-400'}`}><ListFilter size={12} /> Enquete</button>
                      <button onClick={() => setPostType('audio')} className={`flex-1 min-w-[100px] py-2 rounded-lg text-[9px] font-black uppercase tracking-widest transition-all flex items-center justify-center gap-1 ${postType === 'audio' ? 'bg-white dark:bg-slate-800 shadow-sm text-indigo-600' : 'text-slate-400'}`}><Mic size={12} /> Áudio</button>
                  </div>
                  <div className="px-5 pb-4 space-y-4 overflow-y-auto custom-scrollbar flex-1 min-h-0">
                      <div className="space-y-1"><label className="text-[8px] font-black uppercase text-indigo-500 ml-1 tracking-[0.1em]">Descrição (Obrigatória)</label><textarea value={postCaption} onChange={e => setPostCaption(e.target.value)} className="w-full p-4 bg-slate-50 dark:bg-slate-900 border-0 rounded-2xl outline-none font-bold text-sm h-24 transition-all shadow-inner text-slate-800 dark:text-white resize-none" placeholder={postType === 'poll' ? "Pergunta da enquete?" : "Escreva os detalhes aqui..."} /></div>
                      {postType === 'news' && (<div className="space-y-4"><div className="space-y-1"><label className="text-[8px] font-black uppercase text-slate-400 ml-1 tracking-[0.1em]">Adicionar Fotos/Vídeos (Máx 10)</label><label className="w-full h-32 border-2 border-dashed border-slate-200 dark:border-slate-700 rounded-2xl flex flex-col items-center justify-center gap-2 cursor-pointer hover:bg-white dark:hover:bg-slate-800 transition-all bg-slate-50 dark:bg-slate-900 text-center px-4 overflow-hidden"><input type="file" multiple className="hidden" accept="image/*,video/*" onChange={handleFileChange} />{previews.length > 0 ? (<div className="grid grid-cols-5 gap-1 w-full p-2">{previews.slice(0, 10).map((p, i) => (<div key={i} className="aspect-square bg-slate-200 rounded-lg overflow-hidden border border-white/20">{p.type === 'video' ? <div className="w-full h-full flex items-center justify-center bg-indigo-500 text-white"><Video size={10}/></div> : <img src={p.url} className="w-full h-full object-cover" />}</div>))}{previews.length < stagedFiles.length && <div className="aspect-square flex items-center justify-center text-[8px] font-black bg-slate-800 text-white rounded-lg">+{stagedFiles.length - 10}</div>}</div>) : (<><div className="p-3 bg-white dark:bg-slate-700 rounded-full shadow-md"><Plus size={20} className="text-indigo-500" /></div><span className="text-[8px] font-black uppercase text-slate-400 tracking-widest">Selecionar Imagens/Vídeos</span></>)}</label></div><div className="space-y-1"><label className="text-[8px] font-black uppercase text-slate-400 ml-1 tracking-[0.1em]">Anexar Documento (Opcional)</label><label className="w-full py-4 border-2 border-dashed border-blue-200 dark:border-blue-900/30 rounded-2xl flex items-center justify-center gap-3 cursor-pointer bg-blue-50/20 dark:bg-blue-900/10 hover:bg-blue-100/40 transition-all px-4"><input type="file" className="hidden" accept="*" onChange={e => { const f = e.target.files?.[0]; if (f) setStagedDocument(f); }} /><FileText size={20} className={stagedDocument ? "text-green-500" : "text-blue-500"} /><span className={`text-[9px] font-black uppercase truncate max-w-[250px] ${stagedDocument ? "text-green-600" : "text-slate-400"}`}>{stagedDocument ? stagedDocument.name : "Clique para anexar arquivo (PDF, APK, etc)"}</span></label></div></div>)}
                      {postType === 'audio' && (<div className="space-y-4 animate-in zoom-in-95"><label className="text-[8px] font-black uppercase text-slate-400 ml-1 tracking-[0.1em]">Gravar Áudio Direto</label><div className="w-full bg-slate-50 dark:bg-slate-900 border-2 border-slate-200 dark:border-slate-800 rounded-2xl p-6 flex flex-col items-center gap-4">{isRecording ? (<div className="flex flex-col items-center gap-3"><div className="flex items-center gap-2 text-red-500 animate-pulse"><div className="w-3 h-3 bg-red-500 rounded-full"></div><span className="font-black text-xl font-mono">{formatTime(recordingTime)}</span></div><button onClick={stopRecording} className="w-16 h-16 bg-red-500 rounded-full flex items-center justify-center text-white shadow-xl shadow-red-500/30 animate-pulse"><X size={32} /></button><span className="text-[9px] font-black uppercase text-slate-400">Gravando... Toque para parar</span></div>) : recordedAudio ? (<div className="w-full flex flex-col items-center gap-4"><div className="p-4 bg-green-50 text-green-600 rounded-full"><Music size={32}/></div><AudioPlayer url={recordedAudio} /><button onClick={() => {setRecordedAudio(null); startRecording();}} className="flex items-center gap-2 text-[10px] font-black text-indigo-500 uppercase tracking-widest bg-indigo-50 px-4 py-2 rounded-full"><RefreshCw size={12}/> Gravar novamente</button></div>) : (<button onClick={startRecording} className="w-20 h-20 bg-indigo-600 rounded-full flex items-center justify-center text-white shadow-xl shadow-indigo-500/30 hover:scale-105 active:scale-95 transition-all"><Mic size={40} /></button>)}{!isRecording && !recordedAudio && <span className="text-[9px] font-black uppercase text-slate-400">Clique para começar a gravar</span>}</div></div>)}
                      {postType === 'poll' && (<div className="space-y-2 animate-in slide-in-from-top-4 duration-500"><label className="text-[8px] font-black uppercase text-slate-400 ml-1 tracking-[0.1em]">Opções</label><div className="grid gap-1.5">{pollOptions.map((opt, idx) => (<div key={idx} className="flex gap-2 group animate-in slide-in-from-left duration-300" style={{ animationDelay: `${idx * 80}ms` }}><input className="flex-1 px-4 py-2.5 bg-slate-50 dark:bg-slate-900 border-0 rounded-xl text-xs font-bold outline-none focus:ring-2 focus:ring-indigo-500 transition-all text-slate-800 dark:text-white" placeholder={`Opção ${idx + 1}`} value={opt} onChange={e => { const newOpts = [...pollOptions]; newOpts[idx] = e.target.value; setPollOptions(newOpts); }} />{pollOptions.length > 2 && <button onClick={() => { const n = [...pollOptions]; n.splice(idx,1); setPollOptions(n); }} className="p-1.5 text-red-400 hover:text-red-600 transition-all"><MinusCircle size={20}/></button>}</div>))}</div>{pollOptions.length < 5 && <button onClick={() => setPollOptions([...pollOptions, ''])} className="w-full py-2.5 border-0 rounded-xl flex items-center justify-center gap-2 text-indigo-500 bg-indigo-50 dark:bg-slate-900 transition-all hover:bg-indigo-100"><PlusCircle size={16} /><span className="text-[8px] font-black uppercase tracking-widest">Nova Opção</span></button>}</div>)}
                  </div>
                  <div className="p-5 bg-white dark:bg-slate-800 border-t border-transparent shrink-0">
                      {isUploading && (<div className="mb-4 space-y-1.5"><div className="flex justify-between text-[8px] font-black uppercase text-indigo-500"><span className="flex items-center gap-1"><Loader2 size={8} className="animate-spin" /> Processando {stagedFiles.length + (stagedDocument ? 1 : 0)} itens...</span><span>{uploadProgress}%</span></div><div className="w-full h-1 bg-slate-100 dark:bg-slate-700 rounded-full overflow-hidden"><div className="h-full bg-indigo-600 transition-all duration-300" style={{ width: `${uploadProgress}%` }} /></div></div>)}
                      <button onClick={handlePostSubmit} disabled={isUploading || isRecording || !postCaption.trim() || (postType === 'news' && stagedFiles.length === 0 && !stagedDocument) || (postType === 'audio' && !recordedAudio) || (postType === 'poll' && pollOptions.filter(o => o.trim() !== '').length < 2)} className="w-full py-4 bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-700 hover:to-purple-700 text-white rounded-2xl font-black text-[10px] uppercase tracking-[0.2em] shadow-xl active:scale-[0.97] disabled:opacity-30 transition-all flex items-center justify-center gap-2">{isUploading ? <Loader2 className="animate-spin" size={18}/> : <><Send size={18}/> {postType === 'news' ? 'Publicar Tudo' : postType === 'audio' ? 'Lançar Gravação' : 'Lançar Enquete'}</>}</button>
                      {!postCaption.trim() && <p className="text-[8px] font-black text-center text-red-500 uppercase mt-2 tracking-widest">A descrição é obrigatória</p>}
                  </div>
              </div>
          </div>
      )}
